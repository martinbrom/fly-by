...
@section('content')
    ...
    <div class="order-info">
        ...
        <p class="order-duration"><b>Doba:</b> {{ $order->duration }} seconds ({{ round($order->duration / 3600, 1) }}
            hours)</p>
    ...
    <div class="order-aircraft-info row">
        @if(!empty($order->aircraftAirport->aircraft))
            ...
            <div class="col-sm-6">
                ...
                <p class="aircraft-speed"><b>Rychlost letadla:</b> {{ $order->aircraftAirport->aircraft->speed }} km/h
                </p>
                <p class="aircraft-cost"><b>Cena za provoz letadla:</b> {{ $order->aircraftAirport->aircraft->cost }}
                    CZK/km</p>
                <a href="{{ route('admin.aircrafts.show', $order->aircraftAirport->aircraft->id) }}">Zobrazit
                    letadlo</a>
            </div>
        @else
            <div class="col-sm-12">
    ...
    @if($order->confirmed_at == null)
        <hr>
        <form class="mb-1" action="{{ route('admin.orders.confirm-one', $order->id) }}" method="POST"
              id="order-confirm-one-form">
            {{ csrf_field() }}
            <input type="submit" class="btn btn-success" value="Potvrdit objednávku">
        </form>
    @elseif($order->completed_at == null)
        <hr>
        <form class="mb-1" action="{{ route('admin.orders.complete', $order->id) }}" method="POST"
              id="order-complete-form">
            {{ csrf_field() }}
    ...
    @if($order->completed_at == null)
        <form class="mb-1" action="{{ route('admin.orders.destroy', $order->id) }}" method="POST"
